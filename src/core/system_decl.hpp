...
//davidp
	...
	typedef struct{
		std::string slabel;
		float meanipc, stdipc ;
		//float maxipc, minipc;
        	long long meanins , stdins;
		long long meandins, stddins;
        	float meanrtime , stdrtime ;
        	float meanptime , stdptime ;
        	long long meanl1tcm, stdl1tcm;
        	long long  meanl2tcm, stdl2tcm;
        	long long  meanl3tcm, stdl3tcm;
	}labelstats;
	...
	std::multimap<std::string, std::vector<int> > _hashSysTracking;//to save the tracking of a file	
	std::multimap<std::string, int > _hashSysCountTracking;//to count already each executed task types 

	std::list<nanos::WorkDescriptor::wdinfo> _sysStatistics;
	//std::map<std::string, int>  _hashSysStatistics;
	std::list<labelstats> _sysStatisticsGroups;	
	
	std::multimap<std::string,nanos::WorkDescriptor::wdinfo> _hashSysStatistics;	
	std::set<std::string> _wdDescriptionSet;	

	std::multimap<std::string, labelconf> _wdDescriptionMmapConf;//contains pref conf,etc for each task type	
	...
	int wdlistsize;
      ...
      private:
         ...
         unsigned int         	_numPEs;
				 int 										_numPS;//davidp
				 int										_numPB;//davidp
				 unsigned int         	_numIns;//davidp
				 unsigned int					 	_kbPowerBW;//davidp
				 unsigned int					  _applyTrack;//davidp				 
	...
	Lock _hashlock5;
      ...
      public:
#ifdef PAPI
	void addResultToStatistics(WorkDescriptor::wdinfo data);
	void getStatistics();
	void getStatisticsGroups();	
	void printStatistics();
	void writeStatistics();
	void writeStatisticsGroups();

	void addWdDesToSet(std::string wddescription);
	bool checkTaskToMmapConf(std::string wddescription);

	void insertTaskToMmapconf(std::string wddescription);
	int  checkMmapPrefConf(std::string wddescription);
	void resetMmapConfCont(std::string wddescription);
	void increaseMmapPrefConf(std::string wddescription);
	int choseBestPrefConf(std::string wddescription);

	void setMmapPrefConf(int preconf, std::string wddescription);
	void resetMmapPrefConf(std::string wddescription);
	std::string  translateNumberToReal(int reg);
	//int translateRealToNumber(std::string reg);

	void increaseMmapConfCont(std::string wddescription);

	void tangleBLock( std::string wddescription, int thread );
	void untangleBLock( std::string wddescription, int thread );
	void clearBLock( std::string wddescription );
	bool checkPosBLock (std::string wddescription, int thread);	
	bool checkBLock (std::string wddescription);

	void changeModeMmapConf(std::string wddescription, int mode);
	int  checkNumConsecMmapConf(std::string wddescription);
	int  checkModeMmapConf(std::string wddescription);	
	void addResultToGroups(WorkDescriptor::wdinfo data);
	void printGroups();
	void writeDump();
	void writeDumpConfs();
	void writeDumpNumTimesConfs();
	void writeDumpOrderConfs();

	int  checkMmapCountCent(std::string wddescription );
	void increaseMmapCountcent(std::string wddescription);
	void resetMmapCountCent(std::string wddescription);	
	...
	std::vector<std::string> split(const std::string &s, char delim);
         ...
         */
         ...
         int getNumPEs () const;
				 ...
				 //davidp num search and num best
				 int getNumPSearch() const;
				 int getNumPBest() const;
