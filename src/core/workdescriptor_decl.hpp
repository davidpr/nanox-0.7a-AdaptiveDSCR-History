...
#define NUM_EVENTS 3 
   ...
   */
   ...
   class WorkDescriptor : public WorkGroup
   {
	...
	//davidp
	static Lock _dynamiclock;
	static Lock _dynamiclock2;
	...
	typedef struct{
		...
		long long l2tcm;
		long long l3tcm;
		int prefetcherconf;//this is to know always what conf had these stats
	  int phasemode;	
	}wdinfo;

      private:

				int wdprefconfnumber;
				int wdphasemode;
	//pthread_mutex_t count_mutex;//testing
		//davidp variables for papi usage
	wdinfo results  ;
	...
	//davidp
	void writevalues (long_long tvalues[]);
	void addvalues   (long_long tvalues[]);
	//\davidp
